...
**賠率說明**：
- 實際贏分 = ways數 × 賠率 × 下注額
- Wild 僅在軸2~5出現，無法從軸1起頭組成獨立連線，故 Wild 以替代符號形式貢獻獎金
...
|----|----|----|----|----|----|----|----|----|----|----|
| 軸1 | 7 | — | 3 | 4 | 6 | 14 | 18 | 24 | 32 | **108** |
| 軸2 | 8 | 4 | 3 | 4 | 6 | 14 | 18 | 24 | 34 | **115** |
| 軸3 | 8 | 5 | 3 | 4 | 6 | 14 | 18 | 24 | 50 | **132** |
...
|----|----|----|----|----|----|----|----|----|-----|
| 軸1 | 2 | — | 21 | 5 | 14 | 17 | 24 | 27 | **110** |
| 軸2 | 2 | 4 | 9 | 7 | 14 | 17 | 24 | 38 | **115** |
| 軸3 | 2 | 5 | 9 | 7 | 14 | 17 | 24 | 42 | **120** |
| 軸4 | — | 5 | 6 | 11 | 15 | 17 | 26 | 42 | **122** |
| 軸5 | — | 6 | 6 | 11 | 15 | 17 | 26 | 47 | **128** |
...
- WD 同樣均勻間距分佈
- Base Game SC:7/8/8（軸1~3），Free Game SC:2/2/2（降低 Retrigger）
- Scatter 觸發率 ~2.56%（約每 39 把進一次 Free Game）
...
- 2～4 個：每個數量各 20%（60% ÷ 3）
- 5～8 個：每個數量各 9%（36% ÷ 4）
- 9～12 個：每個數量各 0.75%（3% ÷ 4）
- 13～16 個：每個數量各 0.25%（1% ÷ 4）
...
function triggerRandomWilds(grid) {
    ...
    if (roll < 60) {
        ...
        count = randomInt(5, 8);       // 36%: 5~8 個
    } else if (roll < 99) {
        count = randomInt(9, 12);      // 3%: 9~12 個
    } else {
        count = randomInt(13, 16);     // 1%: 13~16 個
    }
...
**技術**：MP3 音檔預載播放（Web Audio API，XHR 預載 buffer）

| 模式 | 檔案 | 風格 |
|------|------|------|
| Base Game BGM | `audio/dragon_wolf/dw-bgm-normal.mp3` | 輕快、森林氣息，琵琶 + 古箏為主 |
| Free Game BGM | `audio/dragon_wolf/dw-bgm-free.mp3` | 懸疑、緊湊，太鼓 + 琵琶快板 |

#### BGM 切換
- Base → Free：進入 Free Game 轉場時先 `bgmStop()` 停 Base BGM，轉場動畫結束後啟動 Free BGM
...
| **big_win** | 大贏音效 | 贏分 ≥ 10× 下注 | `audio/dragon_wolf/free-bigwin.mp3` |
| **scatter_hit** | Scatter 出現聲 | Scatter 落定時 | — |
| **free_spin_intro** | Free Spins 啟動聲 | 進入 Free Spins 轉場 | — |
| **laugh** | 黑白郎君狂笑聲 | FS 轉場 Phase 2 | `audio/dragon_wolf/dw-laugh.mp3` |
| **palm_hit** | 掌力打牆短悶響 | 每個隨機百搭放置時 | — |
| **qigong_1** | 聚氣音效 Tier 1（2秒） | 放置 2~4 個 Wild 時 | `audio/dragon_wolf/qigong-1.mp3` |
| **qigong_2** | 聚氣音效 Tier 2（4秒） | 放置 5~8 個 Wild 時 | `audio/dragon_wolf/qigong-2.mp3` |
| **qigong_3** | 聚氣音效 Tier 3（7秒） | 放置 9+ 個 Wild 時 | `audio/dragon_wolf/qigong-3.mp3` |
...
#### Base Game → Free Game 轉場動畫（約 6.5 秒，`FS_TRANSITION_DURATION = 6500ms`）
...
- 黑龍白狼開始融合，輪廓模糊化
- 大量黑白霧氣從融合點向外擴散
- **此刻響起黑白郎君的狂笑聲**（2000ms 後觸發，持續到動畫結束）
- 音效：能量衝擊波 + 狂笑聲開始
...
| 時間 | 音效 |
|------|------|
| 0～2 秒 | 低沉氣場聚合音（Phase 1 黑龍白狼衝刺） |
| 2000ms | 黑白郎君狂笑聲（標誌性笑聲） |
| 2.0 秒 | 能量衝擊波音效（融合瞬間） |
