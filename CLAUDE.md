...
- **RTP 目標**：96%（實測 95.89%，Base ~50% + Free ~46%）
...
### Symbol 賠率表（黑白龍狼傳 — PR #103 重構，M2=M3、Base 50%/Free 46%）

**Base Game（per-way 倍率 × bet）**
| Symbol | 說明 | 3連 | 4連 | 5連 |
|--------|------|-----|-----|-----|
| M2 | 黑龍 | 0.125 | 0.265 | 0.565 |
| M3 | 白狼 | 0.125 | 0.265 | 0.565 |
| M4 | 憶無心 | 0.08 | 0.105 | 0.285 |
| A1 | A | 0.05 | 0.05 | 0.125 |
| A2 | K | 0.05 | 0.05 | 0.105 |
| A3 | Q | 0.05 | 0.05 | 0.105 |
| A4 | J | 0.04 | 0.04 | 0.084 |

**Free Game（per-way 倍率 × bet）**
| Symbol | 說明 | 3連 | 4連 | 5連 |
|--------|------|-----|-----|-----|
| M1 | 黑白郎君 | 0.077 | 0.214 | 0.380 |
| M4 | 憶無心 | 0.033 | 0.077 | 0.146 |
| A1 | A | 0.010 | 0.033 | 0.077 |
| A2 | K | 0.010 | 0.033 | 0.044 |
| A3 | Q | 0.010 | 0.033 | 0.044 |
| A4 | J | 0.010 | 0.010 | 0.033 |

- Scatter 觸發 Free Spins（固定 10 次，可重觸發 +10 次，上限 50 次）；觸發率 ~2.56%（約 1/39），Base Game SC 數量：軸1=7、軸2=8、軸3=8
- **Free Game Retrigger 機率 ~0.83%**（每局 FS）：Free Reel SC 數量（軸1=5、軸2=6、軸3=6）
...
- **dw-rng.js `_buildReel` fixedSeed（PR #102→#103 v2）**：Free Reel 使用固定 seed 陣列 `[3321, 3418, 3518, 3662, 3824]`（v2 基準：SC:5/6/6 + M1:21/10/11），確保 SC 數量改變時普通符號排列順序不變，RTP 可預期。Base Reel 仍使用動態 seed。
...
- **Free Spins M1 數量（最新，PR #103）**：軸1=21、軸2=10、軸3=11、軸4=6、軸5=6
- **Free Game SC 數量（最新，PR #103）**：軸1=5、軸2=6、軸3=6（Retrigger ~0.83%）
...
- 目標：95.5%~96.5%（PR #103 後：Base 50% + Free 46%）
...
- 實測（PR #103，1000 萬局）：Total **95.89%**（Base **49.91%** + Free **45.99%**）；M1 trigger 12.54%、Retrigger ~0.83%/局 FS
