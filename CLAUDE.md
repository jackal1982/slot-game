...
- **Hash Routing**：`#lobby` 大廳、`#game/slot` 老虎機
- **平台級餘額管理**：餘顽存在 `platform_state` localStorage key，進出遊戲時同步
- **舊版遷移**：自動偵測並遷移 `slotGame_state` 到新 `platform_state` 格式
- **返回按鈕**：僅 IDLE 狀態可用，Spin/Free Spins/Bonus 中 disabled
- **遊戲目錄**：`lobby.js` 中 `_games` 陣列，支援未來擴充新遊戲
...
## 架構概覽
- **全域命名空間**：`window.SlotGame`（Platform, Router, Lobby, Config, State, RNG, Audio, Paylines, Features, Jackpot, Reels, Animations, UI, Main）
...
## 檔案架構
```
index.html          # 主頁面（含大廳 + 遊戲兩個 view）
js/
  platform.js       # ⭐ 平台狀態管理：餘額 API、遊戲生命週期、localStorage 持久化
  router.js         # SPA hash routing（#lobby / #game/slot）、view transition
  lobby.js          # 大廳 UI：遊戲卡片渲染、遊戲目錄、互動事件
  config.js         # ⭐ 核心設定：符號定義、賠率、權重、Reel Strips、賠付線
  main.js           # 遊戲初始化、主流程、cleanup()、returnToLobby()
